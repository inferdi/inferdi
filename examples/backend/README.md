...
Avoid `await using` inside normal HTTP route handlers. Many frameworks can stream or defer response work after the handler returns, so scope disposal must be tied to the actual response lifecycle. Hono has no universal `finish` event; the example is for non-streaming responses and streaming handlers must own cleanup in the stream or `executionCtx.waitUntil`.
