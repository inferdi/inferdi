...
在 **2026-08-17** 记录的公开测试中，默认 checked 模式解析热门单例的中位数为 **6.233 ns/op**。只针对这一场景和记录中的包版本，其他参测容器的中位数是它的 **1.76 到 13.05 倍**。这些数字衡量容器开销，不代表 HTTP 吞吐量或整个应用的性能。[原始结果](https://github.com/inferdi/inferdi/blob/main/benchmarks/results/public-2026-08-17T16-46-00-483Z.json)包含全部八轮数据和环境元数据。
...
构造函数参数的参考说明见[容器选项](../reference/api#容器选项)。本节说明这一选择
对性能的影响。
