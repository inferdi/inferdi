...
- [ ] Изменились `UNDEFINED_MARKER`, `cache`, `regs`, parent lookup или форма `Registration`?
- [ ] Порядок свойств `Registration` изменился с `{kind, lazy, fn, owned}`?
- [ ] Поиск локальной регистрации strict-дерева перенесён после поиска родителя?
...
- [ ] Удалён detach родителя?
- [ ] Дедупликация owned-инстансов перестала сохранять LIFO-порядок первого создания?
