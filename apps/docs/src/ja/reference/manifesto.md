...
- `_disposed`、登録の参照、親の参照、循環チェック、ライフタイムチェック、シングルトンスタックの変更は、すべてキャッシュ高速パスの後に存在します。
- strict ツリーは正確な親チェーンを走査する前にローカル登録を確認します。ルックアップのスナップショットを保持しないため、scope ごとのメタデータや無効化なしで変更が反映されます。
...
公開されるバンドルは gzip 圧縮後 3KB 未満を維持しなければなりません。CI は `pnpm run test:bundle-size` でこの予算を強制します。コア実装または公開ヘルパーにコードを追加する PR では、レビュアーもサイズの変化を確認してください。
...
- [ ] `UNDEFINED_MARKER`、`cache`、`regs`、親ルックアップ、または `Registration` の形が変わった?
- [ ] `Registration` のプロパティ順序が `{kind, lazy, fn, owned}` から変わった?
- [ ] strict ツリーのローカルレジストリ参照が親の参照より後に移された?
...
- [ ] 親のデタッチが削除された?
- [ ] owned インスタンスの重複排除が最初の作成順による LIFO を維持しなくなった?
- [ ] LIFO の破棄順序が変わった?
...
| 非同期 `get()` なし | `get()` は同期のままです。`getAsync()` は同じ同期リゾルバーを Promise で包み、別のレジストリ、キャッシュ、解決レーンを作りません。 |
