...
::: info 参照スニペット
`pnpm run examples:typecheck` は `examples/_shared/container.ts` と `examples/_shared/testing.ts` を検査します。ルート workspace はすべてのフレームワーク依存をインストールしないため、その他のフレームワークスニペットまでは型チェックしません。必要なパターンをアプリケーションへコピーし、依存パッケージを追加して、共有グラフを自身の型に合わせてください。
:::
