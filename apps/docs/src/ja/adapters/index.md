...
2. フレームワークネイティブの場所でスコープを **公開** します。Hono、Koa、Express、Elysia はセットアップ前に公開します。Fastify はセットアップ成功後に `request.di` を公開し、セットアップ失敗時のクリーンアップ中だけ一時的に公開します。クリーンアップフックは公開スロットを参照できますが、エラーハンドラーには構築途中のスコープを渡しません。
